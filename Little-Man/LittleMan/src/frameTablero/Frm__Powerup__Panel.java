...
public class Frm__Powerup__Panel extends JPanel{
	
	private static final long serialVersionUID = 1L;
	private Frm__CONFIG config = Frm__CONFIG.getConfig();
	private Frm__Powerup__Label[][] labels = new Frm__Powerup__Label[config.getCOLUMNAS()][config.getFILAS()];
	private boolean unaVez=true;
	
    public Frm__Powerup__Panel() {
    	setOpaque(false);
		setLayout(null);
		...
		for (int columna = 0; columna < labels.length; columna++) {
			for (int fila = 0; fila < labels[columna].length; fila++) {
				labels[columna][fila] = new Frm__Powerup__Label(600);
	...
	public void actualizarPowerup(int[][] res) {
