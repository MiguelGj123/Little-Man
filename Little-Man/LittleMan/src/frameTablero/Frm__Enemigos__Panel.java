...
public class Frm__Enemigos__Panel extends JPanel{
	
	private static final long serialVersionUID = 1L;
	private Frm__CONFIG config = Frm__CONFIG.getConfig();
	private Frm__Enemigos__Label[][] labels = new Frm__Enemigos__Label[config.getCOLUMNAS()][config.getFILAS()];
	
    public Frm__Enemigos__Panel() {
    	setOpaque(false);
		setLayout(null);
		...
		for (int columna = 0; columna < labels.length; columna++) {
			for (int fila = 0; fila < labels[columna].length; fila++) {
				...
				if (labels[columna][fila].getIcon() != null) {
	                ...
	                // Centrar el Enemigo en la celda
	...
	public void actualizarEnemigos(int[][] res) {
